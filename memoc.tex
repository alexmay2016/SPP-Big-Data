...
*Bei mir fehlen noch die WPs, aber die ergeben sich kanonisch aus den beiden Objectives
}
...
\marc{der einstieg kommt mir etwas zu lang vor; Theoretiker wissen, dass man nach Speicher und Zeit unterscheiden kann, denen muss man nur sagen, dass Krypto bisher immer Zeit macht}
...
\begin{description}
 \item[Analyses of existing LWE/LPN-based Protocols:]
  The idea is to revisit the existing protocols in light of the findings about the memory requirements for LWE and LPN. The goal is to make statements about the security bounds, by verifying if  \marc{to be continued}
